...
// Mark as read (update is_read column) if mark_as_read parameter is passed
if (isset($_GET['mark_as_read'])) {
    $campaign_id = intval($_GET['mark_as_read']);
    
    // Update the is_read column to 1 for the specified campaign
    $update_sql = "UPDATE approved_campaigns SET is_read = 1 WHERE campaign_id = $campaign_id";
...
// Mark as read (update is_read column) if mark_as_read parameter is passed for donations
if (isset($_GET['mark_as_read_donation'])) {
    $donation_id = intval($_GET['mark_as_read_donation']);
    
    // Update the is_read column to 1 for the specified donation
    $update_sql_donation = "UPDATE donations SET is_read = 1 WHERE donation_id = $donation_id";
    
    if ($conn->query($update_sql_donation) === TRUE) {
        // Redirect back to the same page after updating
        header("Location: notifications.php");
        exit();
    } else {
...
<head>
    ...
    <link href="UOngoing.css" rel="stylesheet" >
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <title>Ongoing Campaign</title>
    <style>
        body{
...
<body>
    ...
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.min.js" integrity="sha384-cVKIPhG81rG2K8hPpG8BL4yisSA6zCIpA7G1p1DJVlA20Pvd4S7G+fyIbp7F4cJo" crossorigin="anonymous"></script>
